    ...
    for n from 0 <= n < N + 1:   #  note this is different than loop
        ...
        if ((nWins == 1) and (y[n] < meanO)) or \
           ((nWins == 2) and (y[n, 0] + y[n, 1] < meanO)):
                z[0, n, :] = (0, 1)  #  low state
    m[0]   = _N.sum(z[0, :, 0]) / float(N+1)
    m[1]   = _N.sum(z[0, :, 1]) / float(N+1)
    ...
    for it in xrange(1, NMC+burn):
        ...
        for n from 0 <= n < N + 1:
            if nWins == 1:
                ...
                thr = 1 / (1 + (m[1]/m[0])*exp(0.5*ws[n]*(rsd0*rsd0 - rsd1*rsd1)))
            else:
                for i from 0 <= i < nStates:
                    #  rsd_w1 is 2-component vector (if nStates == 2)
                    rsd_w1 = ((u_w1[i] + smpx[n]) - kw_w1[n])   # residual   
                    trm_w1[i] = -0.5*ws_w1[n]*(rsd_w1*rsd_w1-kw_w1[n]*kw_w1[n])
                    rsd_w2 = ((u_w2[i] + smpx[n]) - kw_w2[n])   # residual   
                    trm_w2[i] = -0.5*ws_w2[n]*(rsd_w2*rsd_w2-kw_w2[n]*kw_w2[n])

                    # trm    = trm_w1 * trm_w2  #  trm is 2-component vector
                thr = 1 / (1 + (m[1]/m[0])*exp(trm_w1[1]+trm_w2[1] - (trm_w1[0] + trm_w2[0])))
            z[it, n, :] = (0, 1)
            if rnds[n] < thr:    #  ratio 0 below thresh
                z[it, n, :] = (1, 0)
