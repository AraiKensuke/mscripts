...
def mcmcMixAR1_vo(burn, NMC, y, zT, nStates=2, nWins=1, r=40, n=400, model="binomial"):
    """
...
#        z[it, :, 1] = zT
        ...
        for n in xrange(N+1):
            ...
            thr = 1 / (1 + (m[1]/m[0])*_N.exp(trm_w1[1]+trm_w2[1] - (trm_w1[0] + trm_w2[0])))
            z[it, n, :] = (0, 1)
#            if rnds[n] < (trm[0] / _N.sum(trm)):
            if rnds[n] < thr:
                z[it, n, :] = (1, 0)
#       print "%(1)d   %(2)d" % {"1" : _N.sum(z[it, :, 0]), "2" : _N.sum(z[it, :, 1])}
